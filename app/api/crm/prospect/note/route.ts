...
import { prospectorQuickNoteWithToken } from "@/app/actions/prospector";
...
export async function POST(req: Request) {
  ...
  const result = await prospectorQuickNoteWithToken(auth.orgId, body);
  if (!result.ok) {
    return NextResponse.json({ ok: false, message: result.error }, { status: 400 });
  }

  return NextResponse.json({ ok: true, noteId: result.data?.noteId });
}
